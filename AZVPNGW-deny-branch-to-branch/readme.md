...
By default, 2 remote sites will be able to communicate if they are connecting via S2S VPN+BGP to the Azure VPN Gateway. Branch to branch communication will hairpin off of the VPN GW. Currently there are no route filtering capabilities on the VPN GW nor can you use NSGs. This lab will illustrate the default behavior as well as an option to use BGP deny spoke to spoke traffic. Note- there are other components you could layer on to segment the environment. IE inbound route filtering, inbound/outbound acls, black hole routing etc. At the end of this lab, each CSR will receive the BGP advertisement but the remote prefix will not make it to the RIB or FIB due to BGP loop prevention. All Azure configurations are done with Azure CLI.
